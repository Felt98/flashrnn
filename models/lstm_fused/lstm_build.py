...
"""
JIT编译并缓存一个 LSTM 的 CUDA fused 版模块到 python中使用的类
被 LSTMFuncGeneratorFused 使用
"""


class _LSTMFusedBuild:
    mod: dict = {}
    ...
    def instance(cls, config: FlashRNNConfig):
        ...
        if cfgdevstr not in cls.mod:
            ...
            name = "lstm_f"
            sources = [
                str(flashrnn_path / "fused" / "lstm" / "lstm.cc"),
                str(flashrnn_path / "fused" / "lstm" / "lstm_fused_forward.cu"),
                str(flashrnn_path / "fused" / "lstm" / "lstm_fused_backward.cu"),
                str(flashrnn_path / "fused" / "lstm" / "lstm_fused_backward_cut.cu"),
            ...
            module = load_parametric_and_test_and_bisect(
                ...
                extra_cuda_cflags=[
                    "-include",
                    str(flashrnn_path / "fused" / "lstm" / "lstm_fused_pointwise.cuh"),
                ],
                error_on_test=False,
            )

            cls.mod[cfgdevstr] = module.LSTMFuncFused(  # 调用.cc里的类构造函数
                True,
                config.batch_size,
                config.hidden_dim,
                config.num_heads,
...
def LSTMFuncGeneratorFused(training, config: FlashRNNConfig):
    # 根据config JIT编译生成一个flashrnn_cuda实例
    flashrnn_cuda = _LSTMFusedBuild.instance(config)
    ...
    class LSTMFunctionFused(torch.autograd.Function):
