...
from .gru_build import GRUFuncGeneratorFused
from ..config import (
    FlashRNNConfig,
    DTYPE_DICT_REV,
)
...
class GRUFused(nn.Module):
    def __init__(
        ...
        """
        Initialize the parameters of the GRU Fused layer.
    ...
    def forward(self, input, states=None):
        ...
        for layer in range(self.num_layers):
            ...
            state = torch.stack([hx], dim=0).unsqueeze(1)  # (NS, 1, B, NH, D)

            # output shape: [1,T+1,B,NH,D]
    ...
    def _impl(self, Wx, R, b, state):
        kernel = GRUFuncGeneratorFused(torch.is_grad_enabled(), self.config)
        state = kernel.apply(
