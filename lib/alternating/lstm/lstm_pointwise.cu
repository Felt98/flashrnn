...
__global__ void FLASHRNNPointwiseForward(const int batch_dim, const int hidden_dim, const int num_heads,
                                         ...
                                         FLASHRNN_DTYPE_G *g_i_out)
{
    ...
    const int weight_idx = col * (hidden_dim * FLASHRNN_NUM_GATES_R) + row + FLASHRNN_NUM_GATES_R * head_idx;
    ...
    const auto iraw =
        add_g(type2float(Wx[i_idx]),
              add_g(type2float(Ry[i_idx]), type2float(b[row + FLASHRNN_NUM_GATES_T * head_idx + 0 * head_dim])));
    const auto fraw =
        add_g(type2float(Wx[f_idx]),
              add_g(type2float(Ry[f_idx]), type2float(b[row + FLASHRNN_NUM_GATES_T * head_idx + 1 * head_dim])));
    const auto zraw =
        add_g(type2float(Wx[z_idx]),
              add_g(type2float(Ry[z_idx]), type2float(b[row + FLASHRNN_NUM_GATES_T * head_idx + 2 * head_dim])));
    const auto zval = tanh_g(zraw);
    const auto ogate = sigmoid_g(
        add_g(type2float(Wx[o_idx]),
              add_g(type2float(Ry[o_idx]), type2float(b[row + FLASHRNN_NUM_GATES_T * head_idx + 3 * head_dim]))));
                                          ...
                                          FLASHRNN_DTYPE_G *dg_b_out)
{
    ...
    const int stride4_base_idx = col * (hidden_dim * FLASHRNN_NUM_GATES_R) + row + FLASHRNN_NUM_GATES_R * head_idx;
