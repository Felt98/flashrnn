...
using lstm::BackwardPass;
using lstm::BackwardPassCut;
using lstm::ForwardPass;
...
class LSTMFunc
{
  ...
  public:
    LSTMFunc(const bool training, const int batch_size, const int hidden_size, const int num_heads)
    ...
    std::vector<Tensor> forward(bool training, Tensor x, Tensor s0, Tensor recurrent_kernel, Tensor bias)
    {
        ...
        // printf("R dim: %d %d %d %d",recurrent_kernel.size(0),recurrent_kernel.size(1),recurrent_kernel.size(2),recurrent_kernel.size(3));

        torch::cuda::synchronize();
        AT_DISPATCH_FLOATING_TYPES_AND_HALF2(
            x.scalar_type(), "LSTMFunc.forward", ([&] {
                                 ...
                                 Tensor gate_cache_i, Tensor ds_new)
    {
        ...
        AT_DISPATCH_FLOATING_TYPES_AND_HALF2(
            recurrent_kernel_t.scalar_type(), "LSTMFunc.backward", ([&] {
                                     ...
                                     Tensor gate_cache_i, Tensor ds_new)
    {
        ...
        torch::cuda::synchronize();
        AT_DISPATCH_FLOATING_TYPES_AND_HALF2(
            recurrent_kernel_t.scalar_type(), "LSTMFunc.backward_cut", ([&] {
...
{
    pybind11::class_<LSTMFunc>(m, "LSTMFunc")
        .def(pybind11::init<const bool, const int, const int, const int>())
        .def("forward", &LSTMFunc::forward)
        .def("backward", &LSTMFunc::backward)
        .def("backward_cut", &LSTMFunc::backward_cut);
}
