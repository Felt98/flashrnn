...
#include "gru_fused_pointwise_base.cuh"
#endif
...
#define _FUSED_KERNEL_MAX_THREADS                                                                                      \
    (WARP_SIZE * FLASHRNN_BACKWARD_WARP_TILING_COUNT_GATE * FLASHRNN_HIDDEN_SIZE / FLASHRNN_NUM_HEADS /                \
     FLASHRNN_BACKWARD_RECURRENT_TILING_COUNT_HIDDEN / FLASHRNN_BACKWARD_WARP_LOOPING_COUNT_HIDDEN *                   \
     FLASHRNN_BACKWARD_WARP_TILING_COUNT_BATCH / FLASHRNN_BACKWARD_WARP_TILING_DIM_HIDDEN)
...
#define _FUSED_KERNEL_MIN_BLOCKS                                                                                       \
    (FLASHRNN_BACKWARD_RECURRENT_TILING_COUNT_HIDDEN * FLASHRNN_BACKWARD_BLOCK_TILING_COUNT_BATCH *                    \
     FLASHRNN_BACKWARD_RECURRENT_TILING_COUNT_GATE * FLASHRNN_BACKWARD_MULTIHEAD_TILING_COUNT)

__global__ void __launch_bounds__(_FUSED_KERNEL_MAX_THREADS, _FUSED_KERNEL_MIN_BLOCKS)
    FLASHRNNCellFusedBackwardCut(const uint steps, const uint batch_dim,
                                 ...
                                 FLASHRNN_DTYPE_G *g_r_inout,    // Output activations (Wx + Ry + b) [], also
                                                                 // contains gate values [T, G-1, B, H] other gates
                                 FLASHRNN_DTYPE_G *g_i_inout,    // [FLASHRNN_NUM_GATES_T, T, B, H]?  input gate
                                 FLASHRNN_DTYPE_G *g_b,          // [FLASHRNN_NUM_GATES_T, T, B, H]?  input gate
                                 const FLASHRNN_DTYPE_S *d_states_ext, FLASHRNN_DTYPE_S *d_states)
{

    uint rgate_offset = FLASHRNN_NUM_GATES_R * FLASHRNN_HIDDEN_SIZE * batch_dim * (steps - 1);
    uint igate_offset = FLASHRNN_NUM_GATES_I * FLASHRNN_HIDDEN_SIZE * batch_dim * (steps - 1);
    uint bgate_offset = (FLASHRNN_NUM_GATES_T - FLASHRNN_NUM_GATES_I) * FLASHRNN_HIDDEN_SIZE * batch_dim * (steps - 1);
    ...
    if ((global_batch_idx < batch_dim) && (global_state_idx < FLASHRNN_HIDDEN_SIZE))
    {
#pragma unroll
        for (uint sub_state_idx = 0; sub_state_idx < FLASHRNN_NUM_STATES; sub_state_idx++)
        {
            ds_inout_local[sub_state_idx] = d_states[FLASHRNN_HIDDEN_SIZE * global_batch_idx + global_state_idx +
        ...
        for (uint t = steps; t >= 1; t--)
        {
            ...
            FLASHRNNPointwiseBackward(
                g_r_inout + rgate_offset + FLASHRNN_NUM_GATES_R * FLASHRNN_HIDDEN_SIZE * global_batch_idx +
                    FLASHRNN_NUM_GATES_R * global_state_idx,
                1,
                g_i_inout + igate_offset + FLASHRNN_NUM_GATES_I * FLASHRNN_HIDDEN_SIZE * global_batch_idx +
                    FLASHRNN_NUM_GATES_I * global_state_idx,
                1, states + state_offset + FLASHRNN_HIDDEN_SIZE * global_batch_idx + global_state_idx,
                FLASHRNN_HIDDEN_SIZE * batch_dim * (steps + 1),
                states + state_offset + FLASHRNN_HIDDEN_SIZE * batch_dim + FLASHRNN_HIDDEN_SIZE * global_batch_idx +
                    global_state_idx,
                FLASHRNN_HIDDEN_SIZE * batch_dim * (steps + 1),
                d_states_ext + state_offset + FLASHRNN_HIDDEN_SIZE * batch_dim +
                    FLASHRNN_HIDDEN_SIZE * global_batch_idx + global_state_idx,
                FLASHRNN_HIDDEN_SIZE * batch_dim * (steps + 1),
                b + global_state_idx * FLASHRNN_NUM_GATES_T + FLASHRNN_NUM_GATES_I, ds_inout_local, 1,
                g_r_inout + rgate_offset + FLASHRNN_NUM_GATES_R * FLASHRNN_HIDDEN_SIZE * global_batch_idx +
                    FLASHRNN_NUM_GATES_R * global_state_idx,
                1,
                g_i_inout + igate_offset + FLASHRNN_NUM_GATES_I * FLASHRNN_HIDDEN_SIZE * global_batch_idx +
            ...
            bgate_offset -= (FLASHRNN_NUM_GATES_T - FLASHRNN_NUM_GATES_I) * FLASHRNN_HIDDEN_SIZE * batch_dim;
...
#pragma unroll
        for (uint sub_state_idx = 0; sub_state_idx < FLASHRNN_NUM_STATES; sub_state_idx++)
        {
            d_states[FLASHRNN_HIDDEN_SIZE * global_batch_idx + global_state_idx +
                     sub_state_idx * FLASHRNN_HIDDEN_SIZE * batch_dim] = ds_inout_local[sub_state_idx];
        }
    }
}

__global__ void gradientBiasAggregationKernel(const uint hidden_size, const uint batch_size, const uint num_heads,
    ...
    if (idx < FLASHRNN_NUM_GATES_T * hidden_size)
    {
                ...
                for (uint b = 0; b < batch_size; b++)
                {
                    res = add_g(res,
                                type2float<FLASHRNN_DTYPE_G>(
                                    gate_gradients_i[(t * batch_size + b) * hidden_size * FLASHRNN_NUM_GATES_I +
                                                     FLASHRNN_NUM_GATES_I * (idx / FLASHRNN_NUM_GATES_T) + gate_idx]));
                }
            }
        ...
        else if (idx < FLASHRNN_NUM_GATES_T * hidden_size)
        {
            ...
            for (uint t = 0; t < steps; t++)
            {
                for (uint b = 0; b < batch_size; b++)
                {
                    res = add_g(res, type2float<FLASHRNN_DTYPE_G>(
                                         gate_gradients_bias_only[(t * batch_size + b) * hidden_size *
                                                                      (FLASHRNN_NUM_GATES_T - FLASHRNN_NUM_GATES_I) +
...
BackwardPassCut::BackwardPassCut(const int batch_size, const int hidden_size, const int num_heads,
...
void BackwardPassCut::Set(const int batch_size, const int hidden_size, const int num_heads,
                          const cublasHandle_t &blas_handle, const cudaStream_t &stream)
{
    data_->batch_size = batch_size;
    data_->hidden_size = hidden_size;
...
int BackwardPassCut::Run(const int steps, const FLASHRNN_DTYPE_R *R_t, const FLASHRNN_DTYPE_B *b,
                         const FLASHRNN_DTYPE_S *s, const FLASHRNN_DTYPE_S *ds_new, FLASHRNN_DTYPE_R *dR,
                         FLASHRNN_DTYPE_B *db, FLASHRNN_DTYPE_S *ds, FLASHRNN_DTYPE_G *g_r, FLASHRNN_DTYPE_G *g_i,
                         FLASHRNN_DTYPE_G *g_bias, FLASHRNN_ACC_DTYPE *d_state_buffer)
{ // [T*N]
    ...
    const uint recurrent_tiling_count_hidden =
        MIN(FLASHRNN_HIDDEN_SIZE / FLASHRNN_NUM_HEADS / FLASHRNN_BACKWARD_WARP_TILING_DIM_HIDDEN /
                FLASHRNN_BACKWARD_WARP_LOOPING_COUNT_HIDDEN, // because of minimal
                                                             // tiling dim tensorop
            MAX(WARP_SIZE * FLASHRNN_HIDDEN_SIZE / FLASHRNN_NUM_HEADS / 1024 / FLASHRNN_BACKWARD_WARP_TILING_DIM_HIDDEN,
                FLASHRNN_BACKWARD_RECURRENT_TILING_COUNT_HIDDEN)); // because the
                                                                   // maximal block
                                                                   // size is 1024

    if (recurrent_tiling_count_hidden != FLASHRNN_BACKWARD_RECURRENT_TILING_COUNT_HIDDEN)
    {
        fprintf(stderr,
        ...
        fprintf(stderr,
                ...
                "% d, WTCH: % d\n ",
                FLASHRNN_BACKWARD_RECURRENT_TILING_COUNT_GATE, FLASHRNN_BACKWARD_RECURRENT_TILING_COUNT_HIDDEN,
                FLASHRNN_BACKWARD_WARP_TILING_COUNT_GATE, FLASHRNN_BACKWARD_BLOCK_TILING_COUNT_BATCH,
...
//     FLASHRNN_BACKWARD_WARP_TILING_DIM_HIDDEN) ", " TOSTRING(FLASHRNN_BACKWARD_WARP_TILING_COUNT_BATCH) ", 1>")
    ...
    const dim3 gridDim(CEIL_DIV(FLASHRNN_HIDDEN_SIZE, BLOCK_SIZE_HIDDEN), CEIL_DIV(batch_size, BLOCK_SIZE_BATCH));

    FLASHRNNCellFusedBackwardCut<<<blockDim, gridDim, 0, stream_K>>>(steps, batch_size, R_t, b, s, g_r, g_i, g_bias,
                                                                     ds_new, ds);
    ...
    auto blas_res = blas<DTYPE>::gemmsb(
        blas_handle_b[0], CUBLAS_OP_N, CUBLAS_OP_T, head_dim, head_dim * FLASHRNN_NUM_GATES_R, batch_size * steps,
        &alpha, s, hidden_size, head_dim, g_r, hidden_size * FLASHRNN_NUM_GATES_R, head_dim * FLASHRNN_NUM_GATES_R,
        &beta_sum, dR, head_dim, FLASHRNN_NUM_GATES_R * head_dim * head_dim, num_heads);
    ...
    {
        gradientBiasAggregationKernel<<<CEIL_DIV(FLASHRNN_NUM_GATES_T * hidden_size, 512), 512, 0, stream_b[1]>>>(
