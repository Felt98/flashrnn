...
def get_runnable_benchmark(
    ...
    def bench_flashrnn(
        ...
        if kernel_spec.function == "nn.GRU":
            nn_gru_dtype_str = kernel_spec.backend.split("-")[-1]
            assert nn_gru_dtype_str in [
                "bfloat16",
                "float32",
                "float16",
            ], f"Invalid dtype for nn.GRU, got {nn_gru_dtype_str}"
            ...
            if kernel_spec.loop:
                ...
                def run_kernel_fn():
                    h0 = torch.zeros(
                        LAYER, B, DH * NH, device=device, dtype=nn_gru_dtype
                    )
                    ...
                    for t in range(T):  # loop over time
                        input_t = pt_in[:, t : t + 1, :]  # shape: [B, 1, DH]
                        out_t, hx = torch_gru(input_t, hx)
                        outputs.append(out_t)

                    out_full = torch.cat(outputs, dim=1)  # [B, T, hidden]
