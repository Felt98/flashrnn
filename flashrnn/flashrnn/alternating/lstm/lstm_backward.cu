...
int BackwardPass::IterateInternal(const FLASHRNN_DTYPE_R *R_t, // [H*4,H]
                                  ...
                                  FLASHRNN_DTYPE_S *ds, // [N,H]
                                  const uint ds_stride, FLASHRNN_DTYPE_G *g_r,
                                  FLASHRNN_DTYPE_G *g_i,
  ...
  auto blas_res = blas<FLASHRNN_DTYPE_R>::gemmsb(
      blas_handle_R, CUBLAS_OP_N, CUBLAS_OP_N, head_dim, batch_size,
      head_dim * FLASHRNN_NUM_GATES_R, &alpha, R_t, head_dim,
      FLASHRNN_NUM_GATES_R * head_dim * head_dim, g_r,
      hidden_size * FLASHRNN_NUM_GATES_R, head_dim * FLASHRNN_NUM_GATES_R,
      &beta_sum, ds, hidden_size, head_dim, num_heads);
