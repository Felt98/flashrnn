...
using gru::BackwardPass;
using gru::BackwardPassCut;
using gru::ForwardPass;
...
class GRUFunc
{
  ...
  public:
    GRUFunc(const bool training, const int batch_size, const int hidden_size, const int num_heads)
                                 ...
                                 Tensor gate_cache_i, Tensor ds_new)
    {
        ...
        AT_DISPATCH_FLOATING_TYPES_AND_HALF2(
            recurrent_kernel_t.scalar_type(), "GRUFunc.backward", ([&] {
                                     ...
                                     Tensor gate_cache_i, Tensor ds_new)
    {
        ...
        torch::cuda::synchronize();
        AT_DISPATCH_FLOATING_TYPES_AND_HALF2(
            recurrent_kernel_t.scalar_type(), "GRUFunc.backward_cut", ([&] {
...
{
    pybind11::class_<GRUFunc>(m, "GRUFunc")
        .def(pybind11::init<const bool, const int, const int, const int>())
        .def("forward", &GRUFunc::forward)
        .def("backward", &GRUFunc::backward)
        .def("backward_cut", &GRUFunc::backward_cut);
}
