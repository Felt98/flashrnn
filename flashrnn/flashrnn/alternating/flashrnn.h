...
class ForwardPass
{
  public:
    ...
    // dimension C).
    // y: [N,H] the t-1 iteration's `y_out` or the initial hidden
    // state if this is the t=0 iteration (typically zeros).
    ...
    // g: [N,H*4] if `training` is `false`, this is scratch space （暂存空间） and should not
    // be used by
    //     the caller. If `training` is `true`, this vector will contain
    //     intermediate（中间的） activations for this iteration which must be provided
    //     as-is to the corresponding（相应的） backward iteration. In either case, a new
    ...
    int Iterate(const cudaStream_t &stream, const FLASHRNN_DTYPE_R *R, const FLASHRNN_DTYPE_B *b,
                const FLASHRNN_DTYPE_W *x, const FLASHRNN_DTYPE_S *s, FLASHRNN_DTYPE_S *s_out, FLASHRNN_DTYPE_G *g_r,
    ...
    int Run(const int steps, const FLASHRNN_DTYPE_R *R, const FLASHRNN_DTYPE_B *b, const FLASHRNN_DTYPE_W *x,
            FLASHRNN_DTYPE_S *s, FLASHRNN_DTYPE_G *g_r, FLASHRNN_DTYPE_G *g_i, FLASHRNN_DTYPE_G *tmp_Ry);

  private:
    int IterateInternal(const FLASHRNN_DTYPE_W *x, const FLASHRNN_DTYPE_R *R, const FLASHRNN_DTYPE_B *b,
                        const FLASHRNN_DTYPE_S *s, const uint s_stride, FLASHRNN_DTYPE_S *s_out,
                        const uint s_out_stride, FLASHRNN_DTYPE_G *g_r, FLASHRNN_DTYPE_G *g_i,
...
class BackwardPass
{
  public:
    ...
    // corresponding
    //     iteration.
    int Iterate(const cudaStream_t &stream, const FLASHRNN_DTYPE_R *R_t, const FLASHRNN_DTYPE_B *b,
                const FLASHRNN_DTYPE_S *s, const FLASHRNN_DTYPE_S *s_new, const FLASHRNN_DTYPE_S *ds_new,
                FLASHRNN_DTYPE_R *dR, FLASHRNN_DTYPE_B *db, FLASHRNN_DTYPE_S *ds, FLASHRNN_DTYPE_G *g_r,
    ...
    int Run(const int steps, const FLASHRNN_DTYPE_R *R_t, const FLASHRNN_DTYPE_B *b, const FLASHRNN_DTYPE_S *s,
            ...
            FLASHRNN_DTYPE_G *g_r, FLASHRNN_DTYPE_G *g_i, FLASHRNN_DTYPE_G *g_b);

  private:
    int IterateInternal(const FLASHRNN_DTYPE_R *R_t, const FLASHRNN_DTYPE_B *b, const FLASHRNN_DTYPE_S *s,
                        const uint s_stride, const FLASHRNN_DTYPE_S *s_new, const uint s_new_stride,
                        const FLASHRNN_DTYPE_S *ds_new, const uint ds_new_stride, FLASHRNN_DTYPE_S *ds,
                        const uint ds_stride, FLASHRNN_DTYPE_G *g_r, FLASHRNN_DTYPE_G *g_i, FLASHRNN_DTYPE_G *g_b);
    struct private_data;
    private_data *data_;
};
...
class BackwardPassCut
{
  public:
    ...
    int Run(const int steps, const FLASHRNN_DTYPE_R *R_t, const FLASHRNN_DTYPE_B *b, const FLASHRNN_DTYPE_S *s,
            const FLASHRNN_DTYPE_S *ds_new, FLASHRNN_DTYPE_R *dR, FLASHRNN_DTYPE_B *db, FLASHRNN_DTYPE_S *ds,
            FLASHRNN_DTYPE_G *g_r, FLASHRNN_DTYPE_G *g_i, FLASHRNN_DTYPE_G *g_bias);

  private:
    int IterateInternal(const FLASHRNN_DTYPE_R *R_t, const FLASHRNN_DTYPE_B *b, const FLASHRNN_DTYPE_S *s,
                        const uint s_stride, const FLASHRNN_DTYPE_S *s_new, const uint s_new_stride,
                        const FLASHRNN_DTYPE_S *ds_new, const uint ds_new_stride, FLASHRNN_DTYPE_S *ds,
                        const uint ds_stride, FLASHRNN_DTYPE_G *g_r, FLASHRNN_DTYPE_G *g_i, FLASHRNN_DTYPE_G *g_bias);
    struct private_data;
    private_data *data_;
};
