...
__global__ void FLASHRNNPointwiseForward(
    ...
    const FLASHRNN_DTYPE_G *Wx, // Precomputed (Wx) vector : [B,NH,NG,D] = [B,NG,D] * NH = [B,NG*H]
    const FLASHRNN_DTYPE_G *Ry, // Precomputed (Ry) vector : [B,NG,D] * NH = [B,NG*H]
    const FLASHRNN_DTYPE_B *b,  // Bias for gates          : [4*H]
    const FLASHRNN_DTYPE_S *s,  // Input  state            : [NS,B,H]
    const uint s_stride,
    ...
    FLASHRNN_DTYPE_G *g_i_out)
{
    ...
    // headID: blockDim.z * blockIdx.z + threadIdx.z
    const int head_idx = (blockDim.z * blockIdx.z + threadIdx.z) * head_dim; // 当前head_dim所在的H的入口

    if (row >= head_dim || col >= batch_dim)
        return;

    // Base index into the Wx and Ry matrices.
    // Both FLASHRNN_NUM_GATES_R == FLASHRNN_NUM_GATES_W = 3

    // FLASHRNN_NUM_GATES_R * head_idx : 真head_dim所在的H的入口（有3个门）
    // hidden_dim * FLASHRNN_NUM_GATES_R : 真H的长度（把所有head dim拼起来的行长度）
    // col * (hidden_dim * FLASHRNN_NUM_GATES_R) + row : 当前head中的 [col*NG][row]
    const int stride3_base_idx = col * (hidden_dim * FLASHRNN_NUM_GATES_R) + row + FLASHRNN_NUM_GATES_R * head_idx;

    // 用于training
    // FLASHRNN_NUM_GATES_I = 4
    const int stride4_base_idx = col * (hidden_dim * FLASHRNN_NUM_GATES_I) + row + FLASHRNN_NUM_GATES_I * head_idx;
    ...
    const auto graw =
        add_g(type2float(Ry[g_idx_r]), type2float(b[row + FLASHRNN_NUM_GATES_T * head_idx + 0 * head_dim]));
    const auto rraw =
        add_g(type2float(Wx[r_idx_w]),
              add_g(type2float(Ry[r_idx_r]), type2float(b[row + FLASHRNN_NUM_GATES_T * head_idx + 1 * head_dim])));
    const auto zraw =
        add_g(type2float(Wx[z_idx_w]),
              add_g(type2float(Ry[z_idx_r]), type2float(b[row + FLASHRNN_NUM_GATES_T * head_idx + 2 * head_dim])));
    const auto nraw =
        add_g(type2float(Wx[n_idx_w]), type2float(b[row + FLASHRNN_NUM_GATES_T * head_idx + 3 * head_dim]));
    float one = 1.;
    ...
    y_new = clip_val_g(y_new, neg_g((float)FLASHRNN_FORWARD_CLIPVAL), (float)FLASHRNN_FORWARD_CLIPVAL);
#endif

    s_out[output_idx + 0 * s_out_stride] = float2type<FLASHRNN_DTYPE_S>(y_new);
}

__global__ void FLASHRNNPointwiseBackward(const int batch_dim, const int hidden_dim, const int num_heads,
                                          ...
                                          FLASHRNN_DTYPE_G *dg_b_out)
{
    ...
    dy_recurrent = clip_val_g(dy_recurrent, neg_g((float)FLASHRNN_GRADIENT_RECURRENT_CLIPVAL),
                              (float)FLASHRNN_GRADIENT_RECURRENT_CLIPVAL);
#endif
    const auto dy_total = add_g(type2float(ds_new[base_idx + 0 * ds_new_stride]), dy_recurrent);
    auto dc_total =
        add_g(type2float(ds_new[base_idx + 1 * ds_new_stride]), type2float(ds_inout[base_idx + 1 * ds_inout_stride]));
    ...
    const auto dg_z = mul_g(d_sigmoid_g(zgate), mul_g(dy_total, sub_g(y_cur, nval)));
    ...
    ds_inout[base_idx + 0 * ds_inout_stride] = float2type<FLASHRNN_DTYPE_S>(dy_prev);
