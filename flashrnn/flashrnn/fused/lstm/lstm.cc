...
using lstm_fused::BackwardPass;
using lstm_fused::BackwardPassCut;
using lstm_fused::ForwardPass;
...
{
  private:
    ForwardPass fw; // 在 fused_forward.cu实现的前向传播封装类
    BackwardPass bw;
    BackwardPassCut bwc;

  public:
    LSTMFuncFused(const bool training, const int batch_size, const int hidden_size, const int num_heads)
    ...
    std::vector<Tensor> forward(bool training, Tensor x, Tensor s0, Tensor recurrent_kernel, Tensor bias)
    {
        ...
        AT_DISPATCH_FLOATING_TYPES_AND_HALF2(
            x.scalar_type(), "LSTMFuncFused.forward", ([&] {
                                 ...
                                 Tensor gate_cache_i, Tensor ds_new)
    {
        ...
        Tensor gate_cache_bias = torch::empty({}, options.dtype(typeToTorchDtype<FLASHRNN_DTYPE_G>()));
#else
        // this is for both recurrent and input base caches, without additional
        // bias-only gates
        Tensor gate_cache_bias =
            torch::empty({time_steps, batch_size, hidden_size * (FLASHRNN_NUM_GATES_T - FLASHRNN_NUM_GATES_I)},
                         options.dtype(typeToTorchDtype<FLASHRNN_DTYPE_G>()));
#endif
#if FLASHRNN_BACKWARD_RECURRENT_TILING_COUNT_GATE > 1
        Tensor d_state_buffer =
            torch::ones({FLASHRNN_BACKWARD_RECURRENT_TILING_COUNT_GATE * FLASHRNN_BACKWARD_BLOCK_TILING_COUNT_BATCH *
                             FLASHRNN_BACKWARD_WARP_TILING_COUNT_BATCH * FLASHRNN_BACKWARD_WARP_LOOPING_COUNT_BATCH *
        ...
        AT_DISPATCH_FLOATING_TYPES_AND_HALF2(
            recurrent_kernel_t.scalar_type(), "LSTMFuncFused.backward", ([&] {
                ...
                res = bw.Run(time_steps, reinterpret_cast<FLASHRNN_DTYPE_R *>(recurrent_kernel_t.data_ptr()),
                             ...
                             reinterpret_cast<FLASHRNN_ACC_DTYPE *>(d_state_buffer.data_ptr()));
            }));
        if (res != 0)
        {
...
#pragma
        ...
        AT_DISPATCH_FLOATING_TYPES_AND_HALF2(
            recurrent_kernel_t.scalar_type(), "LSTMFuncFused.backward_cut", ([&] {
...
{
    pybind11::class_<LSTMFuncFused>(m, "LSTMFuncFused")
        .def(pybind11::init<const bool, const int, const int, const int>())
        .def("forward", &LSTMFuncFused::forward)
        .def("backward", &LSTMFuncFused::backward)
        .def("backward_cut", &LSTMFuncFused::backward_cut);
}
