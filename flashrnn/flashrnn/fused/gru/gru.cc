...
using gru_fused::BackwardPass;
using gru_fused::BackwardPassCut;
using gru_fused::ForwardPass;
...
class GRUFuncFused
{
  ...
  public:
    GRUFuncFused(const bool training, const int batch_size, const int hidden_size, const int num_heads)
    ...
    std::vector<Tensor> forward(bool training, Tensor x, Tensor s0, Tensor recurrent_kernel, Tensor bias)
    {
        ...
        AT_DISPATCH_FLOATING_TYPES_AND_HALF2(
            x.scalar_type(), "GRUFuncFused.forward", ([&] {
                                 ...
                                 Tensor gate_cache_i, Tensor ds_new)
    {
        ...
        AT_DISPATCH_FLOATING_TYPES_AND_HALF2(
            recurrent_kernel_t.scalar_type(), "GRUFuncFused.backward", ([&] {
...
#pragma
        ...
        AT_DISPATCH_FLOATING_TYPES_AND_HALF2(
            recurrent_kernel_t.scalar_type(), "GRUFuncFused.backward_cut", ([&] {
...
{
    pybind11::class_<GRUFuncFused>(m, "GRUFuncFused")
        .def(pybind11::init<const bool, const int, const int, const int>())
        .def("forward", &GRUFuncFused::forward)
        .def("backward", &GRUFuncFused::backward)
        .def("backward_cut", &GRUFuncFused::backward_cut);
}
