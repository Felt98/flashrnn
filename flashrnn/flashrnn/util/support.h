...
#define AT_DISPATCH_FLOATING_TYPES_AND_HALF2(TYPE, NAME, ...)                                                          \
    AT_DISPATCH_SWITCH(TYPE, NAME, AT_DISPATCH_CASE_FLOATING_TYPES_AND_HALF2(__VA_ARGS__))
...
template <> torch::Dtype typeToTorchDtype<float>()
{
    return torch::kFloat;
}

template <> torch::Dtype typeToTorchDtype<at::Half>()
{
    return torch::kHalf;
}
...
template <> torch::Dtype typeToTorchDtype<__half>()
{
    return torch::kHalf;
}
