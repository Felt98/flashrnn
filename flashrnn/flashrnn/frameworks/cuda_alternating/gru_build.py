...
"""
编译并缓存一个 GRU 的 CUDA alternate 版模块到 python中使用的类
被GRUFuncGenerator使用
"""


class _GRUCUDA:
    mod = {}  # 记录特定配置cuda模块的map，所有_GRUCUDA类共享
        ...
        if cfgdevstr not in cls.mod:  # 如果当前配置不在cls.mod中：
            ...
            module = load(
                ...
                sources=[
                    ...
                    str(flashrnn_path / "alternating" / "gru" / "gru.cc"),
                    str(flashrnn_path / "alternating" / "gru" / "gru_forward.cu"),
                    str(flashrnn_path / "alternating" / "gru" / "gru_backward.cu"),
                    str(flashrnn_path / "alternating" / "gru" / "gru_backward_cut.cu"),
                    str(flashrnn_path / "alternating" / "gru" / "gru_pointwise.cu"),
            ...
            # 从module模块中实例化 GRUFunc 对象并缓存到cls.mod[cfgdevstr]
            cls.mod[cfgdevstr] = module.GRUFunc(
                True, config.batch_size, config.hidden_dim, config.num_heads
            )
        # 返回已经加载并初始化好的 GRUFunc 对象
        return cls.mod[cfgdevstr]


"""
CUDA alternate版 kernel的生成函数，生成一个自定义的 PyTorch 自动求导函数类,用于封装 GRU 的前向和反向 CUDA 计算逻辑。
forward调用流程：
    Python 调用GRUFuncGenerator生成flashrnn_cuda
    ↓
    flashrnn_cuda.forward(...) ⬅️ 在 Python 中被调用
    ↓
    module.GRUFunc.forward(...) ⬅️ 通过 JIT 编译返回的模块
    ↓
    flashrnn.cc 中注册了名为 "forward" 的函数 对应 GRUFunc::forward（alternate版）
"""


def GRUFuncGenerator(training, config: FlashRNNConfig):
    flashrnn_cuda = _GRUCUDA.instance(config=config)  # CUDA 后端模块

    # 内部调用cuda kernel类GRUFunction，需要继承自torch.autograd.Function
    class GRUFunction(torch.autograd.Function):
        ...
        def backward(ctx, grad_s):
            ...
            # grad_h0 = grad_h0_raw.squeeze(2)  # [1, 16, 1, 32] -> [1, 16, 32]
