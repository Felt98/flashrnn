...
def _get_kernel(config: FlashRNNConfig):
    if config.backend == "vanilla":
        ...
        def fn(Wx_list, states, R_list, b_list, **kwargs):
            Wx = Wx_list[0]
            R = R_list[0]
            b = b_list[0]
...
def flashrnn_debug(
    ...
    if function == "lstm":
        ...
        s_out_last = last_h[0].squeeze(1)  # [1, B, NH, D]
        c_out_last = last_h[1].squeeze(1)  # [1, B, NH, D]
        h = h[0]
        return h, (s_out_last, c_out_last)
    ...
    # last_h = output[:, :, -1:]
    # h = output[0, :, 1:]  # [B, T, NH, D]
    # s_out_last = last_h[0].squeeze(1)  # [B, NH, D]
    # c_out_last = last_h[1].squeeze(1)  # [B, NH, D]
    # print("h permute: ", h.shape)
    # print("last_h permute: ", last_h.shape)
    # return h, (s_out_last, c_out_last)
