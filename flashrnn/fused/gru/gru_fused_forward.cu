                             ...
                             ACC_DTYPE *gate_buffer)
{
    ...
    const uint gate_warp_local_idx =
        FWTDG * FWLCG *
        ((threadIdx.x % (blockDim.x / FWTCH)) / warpSize); // 当前线程所属 warp 内的 gate 偏移量（相对 block 内）
    const uint gate_blocklevel_idx =
        gate_warp_local_idx + threadIdx.x % FWTDG;     // 当前线程负责的 gate 编号（在 block 范围内）
    const uint gate_warp_overcount = warpSize / FWTDG; // 一个 warp 被划分成多少组处理不同 gate

    const uint rgate_dim = FLASHRNN_NUM_GATES_R * head_dim; // RNN 的 recurrent gate 总维度（gates × hidden per head）
                ...
                for (uint wrch_idx = 0; wrch_idx < FWRCH; wrch_idx++)
                {
                    uint R_offset = FLASHRNN_NUM_GATES_R * multihead_idx * head_dim +
                                    (blockIdx.x * FLASHRNN_NUM_GATES_R * head_dim / FRTCG + gate_warp_local_idx +
                                     wlcg_idx * FWTDG) *
                                        head_dim +
                                    hidden_block_idx * (head_dim / FRTCH) + wtch_idx * (head_dim / FRTCH / FWTCH) +
                                    wrch_idx * FWTDH;
                    nvcuda::wmma::load_matrix_sync(b_frag_cache[wlcg_idx][wrch_idx], R + R_offset, head_dim);
                }
            }
        }
#endif
        ...
        if (head_dim_per_block_shared > 0)
        {
            ...
            if ((threadIdx.x == 0) && (blockIdx.x == 0))
            {
                printf("Assigning R, head_dim_block_shared: %d, local_hidden_offset: "
                       ...
                       head_dim_per_block_shared, local_hidden_offset, local_hidden_warp_dim);
            }
#endif
#pragma unroll
            for (uint j = 0; j < (head_dim_per_block_shared)*FLASHRNN_NUM_GATES_R * head_dim / FRTCG;
                 j += blockDim.y * blockDim.x)
            {
                ...
                uint local_hidden_idx = (local_linear_idx) % (head_dim_per_block_shared);
                uint local_hidden_warp_idx = local_hidden_idx % (local_hidden_warp_dim - local_hidden_offset);
                uint local_hidden_block_idx = local_hidden_idx / (local_hidden_warp_dim - local_hidden_offset);
                uint global_idx =
                    FLASHRNN_NUM_GATES_R * multihead_idx * head_dim +
                    (blockIdx.x * FLASHRNN_NUM_GATES_R * head_dim / FRTCG + local_gate_idx) * head_dim + // gate
                    hidden_block_idx * (head_dim / FRTCH) + local_hidden_offset + local_hidden_warp_idx +
        ...
        for (uint local_it = 0;
             ...
             local_it++)
        {
            const uint local_total_it = local_it * hidden_grid_dim * overthreading_count +
            ...
            const uint local_state_idx =
                (threadIdx.x % (head_dim / FRTCG / FWLCG) + wlcg_idx * (head_dim / FRTCG / FWLCG));
            const uint global_state_idx =
                multihead_idx + (gate_warp_idx - gate_warp_local_idx) / FLASHRNN_NUM_GATES_R + local_state_idx;

            for (uint gate_idx = 0; gate_idx < FLASHRNN_NUM_GATES_T; gate_idx++)
            {
                biases_local[wlcg_idx][gate_idx] = b[FLASHRNN_NUM_GATES_T * global_state_idx + gate_idx];
            }
        }
        ...
        nvcuda::wmma::fragment<nvcuda::wmma::matrix_a, FWTDB, FWTDG, FWTDH, MAT_DTYPE, nvcuda::wmma::row_major> a_frag;
        nvcuda::wmma::fragment<nvcuda::wmma::matrix_b, FWTDB, FWTDG, FWTDH, MAT_DTYPE, nvcuda::wmma::col_major> b_frag;
        ...
        for (uint batch_it = 0; batch_it < BatchIterations; batch_it++)
        {
            ...
            for (uint local_it = 0;
                 local_it < CEIL_DIV(FWLCB * FWTDB * FWTDG * FWLCG, FLASHRNN_NUM_GATES_R * FRTCH * FWTCH * WARP_SIZE);
                 local_it++)
            {
                const uint local_total_it = local_it * hidden_grid_dim * overthreading_count +
                                            hidden_block_idx * overthreading_count + overthreading_idx;
                ...
                if (local_batch_idx < FWLCB * FWTDB)
                {
                    ...
                    for (uint state_idx = 0; state_idx < FLASHRNN_NUM_STATES; state_idx++)
                    {
                        states_local[local_it][state_idx] = states[state_pos + state_idx * B_H * (steps + 1)];
                    }
                }
            }
            ...
            for (uint t = 0; t < steps; t++)
            {
                ...
                for (uint wlcg_idx = 0; wlcg_idx < FWLCG; wlcg_idx++)
                {
                    ...
                    uint state_offset_loc = state_offset + hidden_block_idx * (head_dim / FRTCH);
...
#pragma unroll
                        ...
                        for (uint wrch_idx = 0; wrch_idx < FWRCH; wrch_idx++)
                        {
                            uint midx = wrch_idx * FWTDH + wtch_idx * (head_dim / FRTCH / FWTCH);
                            // change loading here
                            for (uint local_batch_idx = 0; local_batch_idx < FWLCB; local_batch_idx++)
                            {
                                // Load the inputs
                                ...
                                nvcuda::wmma::mma_sync(c_frag[local_batch_idx], a_frag,
                                                       b_frag_cache[wlcg_idx][wrch_idx], c_frag[local_batch_idx]);
                            }
                        }
#endif
                        ...
                        {
                            uint R_idx = R_offset + (midx - (wtch_idx * (head_dim / FRTCH / FWTCH) + FWRCH * FWTDH)) +
                ...
                const uint gate_overthreading_block_idx =
                    (threadIdx.x / ((FLASHRNN_NUM_GATES_R * head_dim * warpSize) / FRTCG / FWTDG / FWLCG));
                // const uint gate_overthreading_block_dim =
                //     (blockDim.x / ((FLASHRNN_NUM_GATES_R * head_dim * warpSize) /
                //                    FRTCG / FWTDG / FWLCG));
                const uint gate_overthreading_count = blockDim.x / ((FLASHRNN_NUM_GATES_R * head_dim) / FRTCG / FWLCG);

#pragma unroll
                for (uint local_it = 0; local_it < CEIL_DIV(FWLCB * FWTDB * FWTDG * FWLCG, FWTCH * WARP_SIZE);
                     local_it++)
                {
                    const uint local_total_it = local_it * gate_overthreading_count +
                    ...
                    if (local_batch_idx < FWLCB * FWTDB)
                    {
                        gate_buffer[(hidden_block_idx * FLASHRNN_HIDDEN_SIZE * FLASHRNN_NUM_GATES_R * batch_dim /
                                         BatchIterations +
                                     (batch_idx + local_batch_idx) * (FLASHRNN_HIDDEN_SIZE * FLASHRNN_NUM_GATES_R) +
                                     FLASHRNN_NUM_GATES_R * multihead_idx + wlcg_idx * FWTDG + gate_idx)] =
                            ((mmul_buffer +
                              (block_batch_idx + local_batch_idx) * (FLASHRNN_NUM_GATES_R * head_dim / FRTCG + FSMP) +
...
#pragma unroll
                            for (uint acc_idx = 1; acc_idx < FRTCH; acc_idx++)
                            {
                                ...
                                acc += ((float *)gate_buffer)[(int_acc_idx * FLASHRNN_HIDDEN_SIZE *
                                                                   FLASHRNN_NUM_GATES_R * batch_dim / BatchIterations +
                        ...
                        for (uint gidx = 0; gidx < FLASHRNN_NUM_GATES_W; gidx++)
                        {
                            gates[FLASHRNN_NUM_GATES_I - FLASHRNN_NUM_GATES_W + gidx] = float2type<FLASHRNN_DTYPE_A>(
                                add_g(type2float(gates[FLASHRNN_NUM_GATES_I - FLASHRNN_NUM_GATES_W + gidx]),
                                      type2float(
                                          Wx[input_idx + FLASHRNN_NUM_GATES_W * local_batch_idx * FLASHRNN_HIDDEN_SIZE +
                        ...
                        FLASHRNNPointwiseForward<Training>(
                            ...
                            g_r_out + B_H * FLASHRNN_NUM_GATES_R * t +
                                (batch_idx + batch_it * blockDim.y * gridDim.y * FWLCB * FWTDB + local_batch_idx) *
                                    FLASHRNN_HIDDEN_SIZE * FLASHRNN_NUM_GATES_R +
    ...
    : data_(new private_data)
{
    ...
    uint num_multihead_streams = num_heads / FLASHRNN_FORWARD_MULTIHEAD_TILING_COUNT;
    data_->stream_K = (cudaStream_t *)malloc(sizeof(cudaStream_t) * num_multihead_streams);
    data_->event_K = (cudaEvent_t *)malloc(sizeof(cudaEvent_t) * num_multihead_streams);
    data_->blas_handle_K = (cublasHandle_t *)malloc(sizeof(cublasHandle_t) * num_multihead_streams);
                      ...
                      const cublasHandle_t &blas_handle, const cudaStream_t &stream)
{
    ...
    data_->main_blas_handle = blas_handle;
    data_->stream = stream;
    uint num_multihead_streams = data_->num_heads / FLASHRNN_FORWARD_MULTIHEAD_TILING_COUNT;
    if (FLASHRNN_NUM_HEADS != data_->num_heads)
    {
        ...
        num_multihead_streams = FLASHRNN_NUM_HEADS / FLASHRNN_FORWARD_MULTIHEAD_TILING_COUNT;
