...
def flashrnn_forward(
    Wx: torch.Tensor,  # [T, B, GI, NH, I]
    states: torch.Tensor,  # [T, B, ST, NH, D] only the first is used for recurrence! T is one (indicates the time dimension)
    ...
    states = states[0]
    states_all[0] = states
    R = R.reshape(1, num_heads, head_dim, num_gates_r * head_dim)  # [1,NH,D,NG*D]
