...
from .lstm_build import LSTMFuncGenerator
from flashrnn.flashrnn_alternating import FlashRNNFuncGenerator


class LSTMCuda(nn.Module):
    def __init__(
        self,
        input_size,
        hidden_size,
        ...
        """
        Initialize the parameters of the LSTM Alternating layer.
    ...
    def forward(self, input, states=None):
        ...
        for layer in range(self.num_layers):
            ...
            recurrent = recurrent.permute(0, 3, 2, 1)  # 这里很坑。。。
    ...
    def _impl(self, Wx, R, b, state):
        kernel = LSTMFuncGenerator(torch.is_grad_enabled(), self.config)
        state = kernel.apply(
