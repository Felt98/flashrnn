...
seq_len = 1
requires_grad = True
csvfilename = f"diff_lstm_cuda_{dtype_str}_T{seq_len}_H{hidden_size}_B{batch}.csv"
...
def sync_from_pytorch_lstm(my_lstm, ref_lstm: nn.LSTM, fused: bool):
    """
...
def sync_grads_R_b(my_lstm, ref_lstm, fused, layer=0):
    ...
    H = my_lstm.hidden_size
    weight_hh_grad = getattr(ref_lstm, f"weight_hh_l{0}").grad  # [4H, H]
    if weight_hh_grad is None:
        print(f"[R] ref_lstm.weight_hh_l0.grad is None")
        return
    ...
    if my_grad_R is None:
        ...
        return
    # ====== 提取bias 的 grad ======
    bias_grad = ref_lstm.bias_hh_l0.grad  # shape: [4H]
